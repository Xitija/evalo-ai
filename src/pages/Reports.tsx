...
const Reports = () => {
  ...
  const { data: upcomingInterviews, isLoading, error } = useMeetings();
  ...
  const filterInterviews = (interviews: any[]) => {
    return interviews.filter((interview) => {
      ...
      //   department === "all" ||
      //   interview.department.toLowerCase() === department.toLowerCase();
      const searchMatch =
  ...
  const filteredUpcoming = Array.isArray(upcomingInterviews)
    ? filterInterviews(upcomingInterviews)
    : [];

//   console.log("Upcoming Interviews:", filteredUpcoming);
  ...
  const InterviewCard = ({ interview }: { interview: any }) => {
    ...
    switch (interview.status) {
      case "upcoming":
        ...
        break;
      case "completed":
        actionButton = <Button variant="outline">View Report</Button>;
        break;
      case "draft":
        actionButton = <Button variant="outline">Continue Setup</Button>;
        break;
      default:
        actionButton = <Button variant="outline">View Report</Button>;
        break;
    ...
    return (
      <Card className="w-full">
        ...
        <CardContent className="pb-3">
          <div className="flex flex-col space-y-1.5">
            <div className="flex items-center text-muted-foreground">
              ...
              <span className="text-slate-700">{interview.date}</span>
            </div>
            {/* <div className="flex items-center text-muted-foreground">
        ...
        <CardFooter>
          <Link to={`/interview-insights/${interview.id}`} className="w-full">
            {actionButton}
          </Link>
        </CardFooter>
      </Card>
            ...
            {filteredUpcoming.length > 0 ? (
              ...
              <div className="text-center py-12 border border-dashed rounded-lg">
                <p className="text-muted-foreground">
                  No upcoming interviews match your filters
                </p>
              </div>
            )}
          </TabsContent>
